...
sub music_news : Local Arg(1)
{
	...
	($c->stash->{recent_releases}, $c->stash->{recent_releases_numitems}, $c->stash->{recent_releases_timestamp}) =
	    $c->model('News')->recent_releases($pager->entries_per_page, ($page - 1) * $pager->entries_per_page);

	$pager->total_entries($c->stash->{recent_releases_numitems});
	$c->stash->{pager} = $pager;
    } 
	...
	($c->stash->{recently_deceased}, $c->stash->{recently_deceased_numitems}, $c->stash->{recently_deceased_timestamp}) =
	    $c->model('News')->recently_deceased($pager->entries_per_page, ($page - 1) * $pager->entries_per_page);

	$pager->total_entries($c->stash->{recently_deceased_numitems});
	$c->stash->{pager} = $pager;
    }
	...
	($c->stash->{recently_brokenup}, $c->stash->{recently_brokenup_numitems}, $c->stash->{recently_brokenup_timestamp}) =
	    $c->model('News')->recently_brokenup($pager->entries_per_page, ($page - 1) * $pager->entries_per_page);

	$pager->total_entries($c->stash->{recently_brokenup_numitems});
	$c->stash->{pager} = $pager;
    }
    else
    {
