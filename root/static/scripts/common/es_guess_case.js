...
String.prototype.toMusicBrainzUpperCase = function() { 
    ...
    {
        x = this[i];
        if (x.charCodeAt(0) >= nonBMPCharCodes.bottom && x.charCodeAt(0) <= nonBMPCharCodes.top) {  // Deseret characters are in plane 1, thus 2 chars wide.  Also avoids CJK issues and other planes 1 - 16 issues.
...
function titleCaseString(inputstring) {
    ...
    if (x.charCodeAt(0) >= nonBMPCharCodes.bottom && x.charCodeAt(0) <= nonBMPCharCodes.top) {  // Unicode hex scalar values for D800-DBFF, the Unicode surrogate code points.
...
function findBasicErrors(ruleSet, type, number, stringBeingFixed, mode, keepUpperCased) {
    ...
    do {  // While not punctuation, this also is used as a useful place to decompose certain presentational Unicode forms, such as precomposed Roman numerals.
        newString.push(stringBeingFixed[strLen-1].replace(/[’ʼʻˮ՚′´﴾﴿‘’‛`″‴⁗ʹʺµ℉℃ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅪⅫⅬⅭⅮⅯⅰⅱⅲⅳⅴⅵⅶⅷⅸⅹⅺⅻⅼⅽⅿⅾ\u003E\u00A0\u0009\u2000-\u2009\u180E\u200A\u2028\u2029\u202F\u205F\u3000\u1680\u000A-\u000D\u232A\u27E8\u003C․\u02D0\u2236\uFF1A\u05C3：\⁄\u01C3！\uFF1F\u2E2E？；\u00B4\u2018-\u201F\u200C\U200D\u2060\u303F\u005F\］\［\（\）\s\uFEFF\27E9\2329]/g, changePunctuation));
    } while (--strLen);
